        ...
        when (type) {
            ActionType.MESSAGE -> message?.let {
                when (messageTarget?.lowercase()) {
                    "everyone", "allplayers" -> Bukkit.broadcastMessage(colorize(replaceRuntimeValues(it, context)))
                    else -> context.sender.sendMessage(colorize(replaceRuntimeValues(it, context)))
                }
            }
